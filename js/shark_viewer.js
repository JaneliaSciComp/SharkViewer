...
SharkViewer.prototype.calculateCameraPosition = function (fov, center, boundingBox) {
	...
	var x2 = Math.floor(boundingBox.xmax - center[0])*2;
	var y1 = Math.floor(center[1] - boundingBox.ymin)*2;
	var y2 = Math.floor(boundingBox.ymax - center[1])*2;
...
SharkViewer.prototype.createNeuron = function(swc_json, color= undefined) {
    ...
    //sphere mode renders 3d sphere
        ...
        for (var node in swc_json) {
            if (swc_json.hasOwnProperty(node)) {
                var sphere = this.generateSphere(swc_json[node]);
                neuron.add(sphere);
                if (this.show_cones){
                    if (swc_json[node].parent != -1) {
                        var cone = this.generateConeGeometry(swc_json[node], swc_json[swc_json[node].parent]);
                        neuron.add(cone);
                    }
                }
            }
...
SharkViewer.prototype.init = function () {
	...
	//Controls
	this.controls = new THREE.OrbitControls(this.camera,  document.getElementById(this.dom_element));
